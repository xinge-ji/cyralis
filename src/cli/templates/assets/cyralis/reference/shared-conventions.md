...
由 `cyralis init/update` 复制到项目的 `.cyralis/reference/shared-conventions.md`。所有 Cyralis 子技能用项目相对路径 `.cyralis/reference/shared-conventions.md` 引用本文件——跨子技能共享但不适合堆在单个技能里的规范的唯一权威版本。
...
`cyralis init/update` 完成后骨架：

```
.cyralis/
├── attention.md           Cyralis 技能启动必读的项目注意事项
...
├── tools/                 跨工作流共享脚本（cyralis init/update 从技能包释放）
└── reference/             共享参考文档（cyralis init/update 从技能包释放）
```
...
- 项目注意事项入口固定为 `.cyralis/attention.md`，所有 Cyralis 子技能启动前必须读取
...
改 `cyralis init/update/reference/shared-conventions.md` 模板，新项目运行 `cyralis init/update` 时带上新版本；已有项目运行 `cyralis update` 同步 `.cyralis/reference/shared-conventions.md`。
...
**issue spec**：report / analysis / fix-note 共用 `doc_type` / `issue` / `status` / `tags`。三者落盘确认态统一写 `status: confirmed`；`severity` / `root_cause_type` / `path` 由对应阶段按需补。
...
5. `cs-docs-neat`：阶段 / 里程碑收尾时同步 `.cyralis/`、README/docs、`.cyralis/attention.md` 和 host skill 投影状态
6. `scoped-commit`
...
1. `cs-learn`：坑点
2. `cs-decide`：暴露的长期约束
3. `cs-docs-neat`：修复暴露了文档、agent 入口或记忆不一致时做项目内整理
4. `scoped-commit`
...
1. `cs-docs-neat`：roadmap 确认落盘或整个 roadmap goal 完成后，同步 `.cyralis/`、README/docs、`.cyralis/attention.md` 和 host skill 投影状态
