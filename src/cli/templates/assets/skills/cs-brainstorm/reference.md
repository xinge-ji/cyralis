...
用于 case 2，路径为 `.cyralis/features/{feature}/{slug}-brainstorm.md`。

case 1 的 handoff note 也复用这个路径和 frontmatter，但只在已经聊出非琐碎技术决策时写；正文只保留"已敲定的设计点"和必要的"选定方向与遗留问题"，删掉空节，不补讨论过程。
...
用于 case 4，路径为 `.cyralis/brainstorms/{slug}/brainstorm.md`。

写的时候保留探索痕迹，但也要把已经形成的约束和分拆信号写清。准备移交 roadmap 时，至少让读者看出：

- 这块为什么值得做
- 已经冒出了哪些潜在 feature / 子模块
- 哪些约束、接口、owner 边界开始成形
- 还缺什么信息才适合正式拆解
