...
- 本地 review / QA / acceptance gate blocked，且缺少继续所需输入
- 同一失败项按本文失败恢复三轮仍不通过
...
- 有候选时只建议用户确认后运行 `cs-learn`；不要自动创建或修改 `.cyralis/compound/`。
...
Final audit passed.
Manual follow-up: <run cs-docs-neat to reconcile docs and project knowledge|none>
```
