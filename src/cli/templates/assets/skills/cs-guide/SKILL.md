...
guidedoc 产物**不在 `.cyralis/` 下**——指南是面向外部读者的可发布产物，和 spec 工件分开。
...
```
python .cyralis/tools/search-yaml.py --dir docs/dev --filter doc_type=dev-guide --filter status=current
python .cyralis/tools/search-yaml.py --dir docs/user --filter doc_type=user-guide --filter component={feature-slug}
```
...
- 用 guide 存放 spec 信息（不变量 / 测试约束 / 根因分析）——这类内容属于 `.cyralis/`
