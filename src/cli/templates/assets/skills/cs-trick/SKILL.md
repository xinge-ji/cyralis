...
> 共享路径与命名约定看 `.cyralis/reference/shared-conventions.md`。产物写入 `.cyralis/compound/`，命名 `YYYY-MM-DD-trick-{slug}.md`，frontmatter 带 `doc_type: trick`。
...
```bash
cyralis memory sync --kind compound --source <写入或更新的 trick 文档路径>
```
...
python .cyralis/tools/search-yaml.py --dir .cyralis/compound --filter doc_type=trick --filter type=library --filter framework~={库名}

# 按技术栈浏览

python .cyralis/tools/search-yaml.py --dir .cyralis/compound --filter doc_type=trick --filter language=typescript --filter status=active

# 归档后查重叠

python .cyralis/tools/search-yaml.py --dir .cyralis/compound --filter doc_type=trick --query "{关键词}" --json
```
