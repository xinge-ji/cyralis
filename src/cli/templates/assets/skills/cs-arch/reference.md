...
```bash
python .cyralis/tools/search-yaml.py --dir .cyralis/compound --filter "doc_type=decision|explore|learning" --query "{模块关键词}"
python .cyralis/tools/search-yaml.py --dir .cyralis/compound --filter doc_type=decision --filter status=active --query "{模块关键词}"
```
