...
Glob `.cyralis/` 发现可用目录和文档，按需取用：

- **`architecture/`** — ARCHITECTURE.md 总入口 + 子系统 doc。改跨模块的东西前看一眼避免违反边界
- **`compound/`** — learning / trick / decision / explore 四类沉淀：
  ```bash
  python .cyralis/tools/search-yaml.py --dir .cyralis/compound --filter doc_type=learning --query "关键词"
  python .cyralis/tools/search-yaml.py --dir .cyralis/compound --filter doc_type=decision --query "关键词"
  python .cyralis/tools/search-yaml.py --dir .cyralis/compound --filter doc_type=trick --query "关键词"
  ```
...
最终路径：`.cyralis/features/YYYY-MM-DD-{slug}/{slug}-ff-note.md`，日期用今天。
...
- 改动涉及 3 个以上子系统
- 需要引入新术语或和现有术语冲突
- 要动 `.cyralis/architecture/` 既定的模块边界
- 用户追加的要求让范围翻倍
