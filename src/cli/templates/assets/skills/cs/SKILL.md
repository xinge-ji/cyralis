...
**本技能不做事**：不写 spec / 不读写 `.cyralis/` 下内容产物 / 不替子技能跑流程。产出只有"建议触发哪个子技能"。
...
> 项目已初始化的话更详细总览看 `.cyralis/reference/system-overview.md`。

---

## 场景路由表

匹配用户的话到表里某行，告诉用户："你这个诉求建议走 `{目标 skill}`，因为 {一句话理由}"。

| 用户说什么 / 想做什么 | 路由到 |
|---|---|
| 仓库还没有 `.cyralis/` | **先执行 `cyralis init`**——所有其他 cs-* 都依赖这个目录 |
...
- **不重复体系总览细节**——`.cyralis/reference/system-overview.md` 才是权威完整版
- **不绕过 `cyralis init`**——仓库没接入就先初始化
