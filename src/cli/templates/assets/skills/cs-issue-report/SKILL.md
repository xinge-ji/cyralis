...
涉及快速通道判断时，同时读取 `.cyralis/reference/issue-quick-lane.md`。快速通道不是"感觉简单就直接改"，而是低风险、单 owner、证据明确的 compact debugging lane。
...
3. **快速通道判断（唯一正式判定点）**——按用户线索**读一下相关代码**（Grep / Read 定位），再对照 `.cyralis/reference/issue-quick-lane.md`：
