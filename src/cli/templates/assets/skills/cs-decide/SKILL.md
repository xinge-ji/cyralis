...
python .cyralis/tools/search-yaml.py --dir .cyralis/compound --filter doc_type=decision --filter category=constraint --filter status=active

# 归档后查重叠

python .cyralis/tools/search-yaml.py --dir .cyralis/compound --filter doc_type=decision --query "{关键词}" --json
```
